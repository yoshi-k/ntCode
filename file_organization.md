...
| `llm.py` | Active provider and conversation state. `llm` is the active provider: `providers.anthropic.AnthropicProvider` for Claude (native tool use), or a legacy text-protocol `LLM` (`OpenAILLM`, `DummyLLM`) with a single `call()` method. `switch_provider()` replaces it. `SessionHeader` holds the system prompt and documentation files (`system_with_docs()` for native providers); `ConversationManager` stores the task conversation as `core.types.Message`s. `execute_llm_call()` wraps legacy `LLM`s only, turning their exceptions into error text. |
