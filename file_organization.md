...
| `tool_format.py` | **Provider-aware tool formatter and parser.** `_detect_family(provider, model)` maps a model name to a format family (`ntcode`, `xml`, or `json_block`). `format_tools_for_provider(provider, model, tool_registry)` returns the `{{TOOLS}}` replacement string in the syntax the target model expects. `get_parser_for_provider(provider, model)` returns the matching parser callable `(text) -> List[Tuple[name, args]]`. Three built-in families: **ntcode** — original `tool: NAME({...})` protocol (Claude, GPT-*); **xml** — `<tool_call>{...}</tool_call>` blocks (Qwen2.5-Instruct, Qwen3); **json_block** — fenced ` ```json ` blocks (Mistral, Mixtral). Adding a new family requires one formatter function, one parser function, and entries in `_FORMAT_REGISTRY` / `_PARSER_REGISTRY` / `_detect_family()`. |
