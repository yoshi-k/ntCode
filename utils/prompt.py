...
3. Replace the ``{{TOOLS}}`` placeholder with :data:`TOOLS_NOTE`, a short
   note that the tools are defined separately.  The tool definitions
   themselves are sent by the provider: in the API request's ``tools`` field
   (native tool calling), or appended to the system prompt in the configured
   text dialect (providers.text_tools).
...
def build_system_prompt() -> str:
    """Load, resolve, and return the system prompt (cached; see invalidate_cache).
    ...
    # Step 2: the note about tools goes at the one {{TOOLS}} in the stub.
    if "{{TOOLS}}" in resolved:
        resolved = resolved.replace("{{TOOLS}}", TOOLS_NOTE, 1)
    else:
        logger.warning("prompt: stub contains no {{TOOLS}} placeholder — tools note not added")
